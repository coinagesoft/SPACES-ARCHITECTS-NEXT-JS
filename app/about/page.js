...
export default function AboutPage() {
    return ( <
        ...
        <
        Image src = { assets.about.heroImages[0] }
        alt = "Studio interior"
        ...
        p className = { styles.kicker } > { aboutCopy.kicker } < /p> {
            aboutCopy.intro.map((paragraph) => ( <
                p key = { paragraph }
                className = { styles.bodyCopy } > { paragraph } <
                /p>
            ))
        } <
        ...
        <
        Image src = { assets.about.founderPhoto }
        alt = { aboutCopy.founder.name }
        fill sizes = "(min-width: 768px) 30vw, 90vw"
        className = "object-cover" /
        >
        <
        /div> <
        ...
        <
        h2 >
        Founder: < span > { aboutCopy.founder.name } < /span> <
        /h2> <
        div className = { styles.founderBio } > {
            aboutCopy.founder.bio.map((paragraph) => ( <
